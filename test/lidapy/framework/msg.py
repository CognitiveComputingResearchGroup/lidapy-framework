...
from lidapy.util.comm import LocalCommunicationProxy, MsgUtils
from lidapy.util.logger import ConsoleLogger
from time import sleep


class FrameworkTopicTest(unittest.TestCase):
    @classmethod
    def setUpClass(cls):
    ...
    def test_subscribe(self):
        ...
        def test_random_topic_name_generation(self):
            topic_name_prefix = "Topic_"
            topic_name_length = 20
            topic_name \
                = FrameworkTopic.generate_random_topic_name(prefix=topic_name_prefix,
                                                            length=topic_name_length)
            self.assertIsNotNone(topic_name)
            self.assertEqual(len(topic_name), topic_name_length)
